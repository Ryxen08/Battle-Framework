class_name SonicAtkArF extends PlayerAction
...
func start(old_state: int) -> void:
	super(old_state)
	brain.play_anim(&"AtkArF", true)
	if brain.atk_stk_input_raw.length() > PlayerInput.DEADZONE:
		kb_aim = brain.atk_stk_input_raw
	else:
		kb_aim = brain.last_move_input_raw
...
func update(delta: float) -> void:
	...
	if brain.frame == 9.0:
		hitboxes["A/0"].activate({aim = kb_aim})
		hitboxes["A/1"].activate({aim = kb_aim})
	
	if brain.frame >= 10.0 && brain.frame <= 14.0:
		if brain.velocity.y <= 5.0 && can_boost && !brain.is_on_floor():
	...
	if brain.is_on_floor():
		if brain.frame >= 7.0 && brain.frame <= 13.0:
			brain.land(9.0, 3.0)
		else:
			brain.land()
